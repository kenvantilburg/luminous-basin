...
list_m = np.arange(3,40,0.1)
m = list_m[i_m]  # axion mass [keV]
E_cut_min = 3.999 # minimum energy (below this value, MCMC model not valid)
E_cut_max = 150.001 # maximum energy
...
df_data = load_data(m,sigma_E,good_time_ints,livetime,bins_t,bins_E,n_sigma_E,list_file_events,file_box_centers,list_file_arf,E_cut_min,E_cut_max)
...
df_data_not_m = load_data_not_m(m,sigma_E,good_time_ints,livetime,bins_t,bins_E,n_sigma_E,list_file_events,file_box_centers,list_file_arf,E_cut_min,E_cut_max)
...
for sample in sampler.sample(p0, iterations=max_n):
    ...
    print('sampler.iteration ='+str(sampler.iteration))
