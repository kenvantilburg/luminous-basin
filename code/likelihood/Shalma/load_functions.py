...
def load_arf_m(list_file_arf,bins_E,df_box,m,sigma_E,n_sigma_E=2):
    ...
    df_arf_m = df_arf_m.drop(columns=['index'])
    return df_arf_m
...
def indexed_events(dat,bins_t,bins_E,df_box,m=None,sigma_E=0.166,n_sigma_E=3):
    ...
    If m=None, then all data are indexed. If the axion mass 'm' is specified, then only data with m/2 - n_sigma_E*sigma_E < E < m/2 + n_sigma_E*sigma_E are indexed.
