...
dir_production = dir_home+'ceph/LuminousBasin/data/production/'
...
def T_flux_template(t,ra,dec,ra_sun_0,dec_sun_0,delta_ra_sun,delta_dec_sun,t_min,duration):
    ...
    T = np.zeros(theta.shape)+1e-20
    T += ((theta > theta_sun) & (theta < np.pi/2)) * 3 * np.pi / 2 * np.sin(theta_sun)**3 * np.sin(theta)**-3
